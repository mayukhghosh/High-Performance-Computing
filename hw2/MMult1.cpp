...
void MMult1(long m, long n, long k, double *a, double *b, double *c) {
  ...
  for (long y = 0; y< m; y=y+BLOCK_SIZE){
    for (long x = 0; x< n; x=x+BLOCK_SIZE){
      for (long j = x; j < x+BLOCK_SIZE; j++) {
        for (long p = 0; p < k; p++) {
          for (long i = y; i <y+ BLOCK_SIZE; i++) {
            double A_ip = a[i+p*m];
            double B_pj = b[p+j*k];
            double C_ij = c[i+j*m];
            C_ij = C_ij + A_ip * B_pj;
...
int main(int argc, char** argv) {
  ...
  for (long p = PFIRST; p < PLAST; p += PINC) {
    ...
    double time = t.toc();
    double flops = 2*m*n*k*NREPEATS/(time*1000000000);; // TODO: calculate from m, n, k, NREPEATS, time
    double bandwidth = 4*8*m*n*k*NREPEATS/(time*1000000000); // TODO: calculate from m, n, k, NREPEATS, time
